        ...
        <div class="main-bar row">
            ...
            <div class="col-lg-6 col-sm-8 col-xs-12">
                <ol class="breadcrumb float-xs-right  nav_breadcrumb_top_align">
                    ...
                    <li class="active breadcrumb-item">Manage Section</li>
                </ol>
            </div>
        </div>
    </header>
                    ...
                    <div class="table-toolbar">
                        <div class="btn-group">
                            <button class="btn btn-raised btn-info adv_cust_mod_btn lightspeedin"
                                    data-toggle="modal" data-target="#modal-lightspeedin"><i class="fa fa-plus"></i> Add New Section
                            </button>
                                                          ...
                                                          <table>
                                                            ...
                                                            @foreach($sections as $section)
                                                              ...
                                                              <tr role="row">
                                                                ...
                                                                <td>
                                                                    <a class="edit" data-toggle="tooltip" data-placement="top" title="Edit" onclick="edit_section( {{$section->id}})" href="javascript:;"><i class="fa fa-pencil text-warning"></i></a>&nbsp; &nbsp;
                                                                    <a class="delete" onclick="delete_section( {{$section->id}})" href="javascript:" data-toggle="tooltip" data-placement="top" title="Delete"><i class="fa fa-trash text-danger"></i></a>
                                                                </td>
                                                              </tr>
                                                            <?php $ii++; ?>
                                                                @endif
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header bg-success">
                    <h4 class="modal-title text-white" id="modalLabelzoom">Update Section</h4>
                </div>
...
<script type="text/javascript">
    $("body").on("click",".add-section",function(e){
        $(this).parents("form").ajaxForm(addSection);
    });
    ...
    var addSection = {
        complete: function(response)
        {
            if($.isEmptyObject(response.responseJSON.error)){
                swal({
                    title: "Success.",
                    text: "New Section Successfully Added",
                    type: "success",
                    allowOutsideClick: false
                }).then(function(res) {
//                    $('#editable_table').DataTable().clear().destroy();
//                    var classes = response.responseJSON['classes'];
...
//                                  if(value.class_id === val.class_id){
...
////                                          '<td>'+section_name+'</td>'+
////                                          '<td>'+
////                                          '<a class="edit" data-toggle="tooltip" data-placement="top" title="Edit" onclick="edit_class('+section_id+')" href="javascript:;"><i class="fa fa-pencil text-warning"></i></a>&nbsp; &nbsp;'+
////                                          '<a class="delete" onclick="delete_class('+section_id+')" href="javascript:" data-toggle="tooltip" data-placement="top" title="Delete"><i class="fa fa-trash text-danger"></i></a>'+
////                                          '</td>'+
////                                          '</tr>'
////                                      );
//
            ...
            success: function (response) {
                if (response.error){
                    ...
                    jQuery("#modal-zoomin").show(function () {
                        $(".print-clsss").html(' {!! Form::open(array('class'=>'ajax','id'=>'add_section','url'=>'/update_section','enctype'=>'multipart/form-data')) !!}' +
                            '<div class="alert alert-danger print-error-msg" style="display:none">'+
                            '<ul></ul>'+
                            '</div>'+
                            '<input type="hidden" value='+response.id+' name="section_id">'+
                            '<div class="form-group row">'+
                            '<div class="col-xl-3 text-xl-right">'+
                            '<?php echo Form::label('section_name', 'Section Name: ',['class'=>'form-control-label'])?>'+
                            '</div>'+
                            '<div class="col-xl-5">'+
                            '<input type="text" name="section_name" value='+response.section_name+' placeholder="enter new section name" class="form-control">'+
                            '</div>'+
                            '</div>'+
                            '<div class="form-actions form-group row">'+
                            '<div class="col-xl-8 push-xl-4">'+
                            '<input type="submit" value="Conform Update" class="btn btn-primary update-section">'+
                            '</div>'+
                            '</div>'+
                            '{!! Form::close() !!}');
                    });
